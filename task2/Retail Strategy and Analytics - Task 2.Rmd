...
measureOverTime <- data[, .(
  totSales = sum(TOT_SALES),                         # total revenue
  nCustomers = uniqueN(LYLTY_CARD_NBR),              # unique customers
  nTxnPerCust = uniqueN(TXN_ID) / uniqueN(LYLTY_CARD_NBR),  # transactions per customer
  nChipsPerTxn = sum(PROD_QTY) / uniqueN(TXN_ID),    # quantity per transaction
  avgPricePerUnit = sum(TOT_SALES) / sum(PROD_QTY)   # price per unit
...
calculateCorrelation <- function(inputTable, metricCol, storeComparison) {
  ...
  for (i in storeNumbers) {
    ...
    # Extract data for trial store and comparison store
    store1 <- inputTable[STORE_NBR == storeComparison, .(YEARMONTH, metric = get(metricCol))]
    store2 <- inputTable[STORE_NBR == i, .(YEARMONTH, metric = get(metricCol))]
    
    # Merge on YEARMONTH to align time series
    merged <- merge(store1, store2, by = "YEARMONTH", suffixes = c("_trial", "_control"))
    
    # Calculate correlation
    corr <- cor(merged$metric_trial, merged$metric_control)
...
calculateMagnitudeDistance <- function(inputTable, metricCol, storeComparison) {
  ...
  for (i in storeNumbers) {
    ...
    store1 <- inputTable[STORE_NBR == storeComparison, 
                         .(YEARMONTH, metric = get(metricCol))]
    
    store2 <- inputTable[STORE_NBR == i, 
                         .(YEARMONTH, metric = get(metricCol))]
    
    merged <- merge(store1, store2, by = "YEARMONTH",
                    suffixes = c("_trial", "_control"))
    
    calculatedMeasure <- data.table(
      Store1 = storeComparison,
      Store2 = i,
      YEARMONTH = merged$YEARMONTH,
      measure = abs(merged$metric_trial - merged$metric_control)
    )
  ...
  distTable <- merge(calcDistTable, minMaxDist, 
                     by = c("Store1", "YEARMONTH"))
  
  distTable[, magnitudeMeasure := 1 - 
              (measure - minDist) / (maxDist - minDist)]
  
  distTable[is.nan(magnitudeMeasure), magnitudeMeasure := 1]
...
score_nSales <- merge(corr_nSales, magnitude_nSales, 
                      by = c("Store1", "Store2"))[
                        , scoreNSales := corr_weight * corr_measure + 
...
# Exclude the trial store itself and rank
score_Control <- score_Control[Store2 != trial_store][order(-finalControlScore)]

# Select the best control store (top of remaining list)
control_store <- score_Control[1, Store2]
...
ggplot(pastCustomers, aes(TransactionMonth, nCustomers, color = Store_type)) +
 geom_line() +
 labs(x = "Month of operation", y = "Number of customer", title = "Customer count by month")
```
...
scalingFactorForControlCust <- 
  preTrialMeasures[STORE_NBR == trial_store & YEARMONTH < 201902, 
                   sum(nCustomers)] /
...
percentageDiff <-  merge(scaledControlCustomers[, c("YEARMONTH", "controlCustomers")],measureOverTimeCusts[STORE_NBR == trial_store,
...
aes(xmin = min(TransactionMonth), xmax = max(TransactionMonth), ymin = 0 , ymax = Inf, color = NULL), show.legend = FALSE) +
 geom_line() +
 labs(x = "Month of operation", y = "Customer number", title = "Customer Count by month")
```
...
"controlCustomers")], measureOverTime[STORE_NBR == trial_store, c("nCustomers", "YEARMONTH")], by = "YEARMONTH")[, percentageDiff := 
abs(controlCustomers-nCustomers)/controlCustomers]
...
#### Calculate the absolute percentage difference between scaled control sales and trial sales
percentageDiff <- merge(scaledControlCustomers[, c("YEARMONTH",
"controlCustomers")],measureOverTime[STORE_NBR == trial_store,
c("nCustomers", "YEARMONTH")], by = "YEARMONTH"
)[, percentageDiff :=
abs(controlCustomers - nCustomers)/controlCustomers]
...
ggplot(trialAssessment, aes(TransactionMonth, numberCustomers, color = Store_type)) +
  geom_rect(
    data = trialAssessment[YEARMONTH > 201901 & YEARMONTH < 201905],
    aes(xmin = min(TransactionMonth), xmax = max(TransactionMonth), ymin = 0, ymax = Inf, color = NULL), show.legend = FALSE) + 
geom_line() +
